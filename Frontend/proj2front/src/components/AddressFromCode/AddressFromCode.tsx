...
const AddressFromCode = () => {
  const [location, setLocation] = useState({
    ...
    title: '',
    message:
      'Enter Code to Find Location Below or Enter a Valid URL in the Address Bar',
  })
  ...
  let inputForm = (
    <InputGroup className="code-search-form justify-content-center">
      <Form.Control
        type="text"
  ...
  let toDisplay = (
    <>
      <div className="container">
        ...
        <div className="row justify-content-center">{location.message}</div>
        <div className="row justify-content-center">{inputForm}</div>
  ...
  let searchForLocation = async (codeToSend: string) => {
    try {
      ...
      setLocation({ ...loc, title: title, message: '' })
      setCode('')
    } catch (error:any) {
      console.log(error)
